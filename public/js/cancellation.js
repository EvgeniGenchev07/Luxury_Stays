...
document.querySelector("#status-icon").style.backgroundColor = "#6D6E78";
document.querySelector("#status-icon").innerHTML = InfoIcon;
document.querySelector("#status-text").textContent= 'Your request is processing.';
document.querySelector('#spinner').classList.remove('hidden');
const urlParams = new URLSearchParams(window.location.search);
const id = urlParams.get('request');
if (id !== null) {
    fetch('https://europe-central2-luxurystayskapanaplovdiv.cloudfunctions.net/cancelBooking', {
        method: 'POST',
        body: id,
    }).then((response) => {
        if(response.ok) {
            ...
            document.querySelector("#status-text").textContent = 'Cancellation succeeded';
            document.querySelector('#spinner').classList.add('hidden');
            document.querySelector("#details-table").classList.remove('hidden');
            document.querySelector("#intent-id").textContent = id;
            document.querySelector("#intent-status").textContent = 'cancelled';
        }
        else SetErrorStatus();
    }).catch((error) => {
        SetErrorStatus();
...
function SetErrorStatus(){
    ...
    document.querySelector('#spinner').classList.add('hidden');
    document.querySelector("#intent-id").textContent = id;
    document.querySelector("#intent-status").textContent = 'failed';
    const btn = document.querySelector("#retry-button");
    btn.classList.remove('hidden');
    btn.addEventListener('click', () => {
        location.replace("https://luxurystayskapanaplovdiv.web.app/cancellation?request=" + id);
    })
}
